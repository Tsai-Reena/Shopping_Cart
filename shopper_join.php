...
if(isset($_POST["action"])&&($_POST["action"]=="join")){
	...
	//找尋帳號是否已經註冊
	$query_RecFindUser = "SELECT Username FROM shopper WHERE Username='{$_POST["m_username"]}'";
	$RecFindUser=$db_link->query($query_RecFindUser);
	if ($RecFindUser->num_rows>0){
		header("Location: shopper_join.php?errMsg=1&username={$_POST["m_username"]}");
	}else{
	//若沒有執行新增的動作	
		//echo $_POST["m_address"];
		$query_insert = "INSERT INTO `shopper` ( `Username`, `Password`, `Gender`, `Birthday`, `Email`, `Phone`, `Address`, `Jointime`) VALUES (?, ?, ?, ?, ?, ?, ?, NOW())";
		$stmt = $db_link->prepare($query_insert);
		$stmt->bind_param("sssssss", 
			//GetSQLValueString($_POST["m_name"], 'string'),
			GetSQLValueString($_POST["m_username"], 'string'),
			GetSQLValueString($_POST["m_passwd"], 'string'),
		...
		$stmt->close();
		$db_link->close();
		header("Location: shopper_join.php?loginStats=1");
	}
}
?>
<html>
      ...
      <tr valign="top">
        <td class="tdrline"><form action="" method="POST" name="formJoin" id="formJoin" onSubmit="return checkForm();">
          ...
          <div class="dataDiv">
            ...
            <p><strong>性　　別</strong>：
            <input name="m_sex" type="radio" value="F" checked>女
            <input name="m_sex" type="radio" value="M">男
            <font color="#FF0000">*</font></p>
            <p><strong>生　　日</strong>：
