...
if(isset($_POST["action"])&&($_POST["action"]=="join")){
	...
	//找尋帳號是否已經註冊
	$query_RecFindUser = "SELECT Username FROM seler WHERE Username='{$_POST["m_username"]}'";
	$RecFindUser=$db_link->query($query_RecFindUser);
	if ($RecFindUser->num_rows>0){
		header("Location: seller_join.php?errMsg=1&username={$_POST["m_username"]}");
	}else{
	//若沒有執行新增的動作	
		$query_insert = "INSERT INTO `seler` (`Username`, `Password`, `Gender`, `Email`, `Phone`, `Address`) VALUES (?, ?, ?, ?, ?, ?)";
		...
		$stmt->close();
		$db_link->close();
		header("Location: seller_join.php?loginStats=1");
	}
}
?>
<html>
