...
if(isset($_POST["username"]) && isset($_POST["passwd"])){
	//繫結登入會員資料
	$query_RecLogin = "SELECT `Username`, `Password`, `ID` FROM `seler` WHERE `Username`=?";
  ...
  $password_hash = password_hash($passwd, PASSWORD_DEFAULT);
	if(password_verify($_POST["passwd"], $password_hash)){
		//計算登入次數及更新登入時間
		$query_RecLoginUpdate = "UPDATE `seler` SET `Login`=1, `logintime`=NOW() WHERE `Username`=?";
    ...
    // echo password_verify($_POST["passwd"],$passwd);
		header("Location: sellerlogin.php?errMsg=1");
    // echo "Fail";
	}
}
?>
...
<table width="780" border="0" align="center" cellpadding="4" cellspacing="0">
  ...
  <tr>
    <td class="tdbline"><table width="100%" border="0" cellspacing="0" cellpadding="10">
      <tr valign="top">
        <td class="tdrline"><p class="title">歡迎光臨3C交易平台</p>
...
記住我的帳號密碼。</p>
          ...
          <p>註冊帳號免費又容易</p>
          <p align="right"><a href="seller_join.php">馬上申請會員</a></p>
